...
bool SynchronousTxnGenerator::Loop() {
  ...
  if (workload_->name() == "smallbank" && !duration_reached) {
    // Update the SmallBank workload for sunflower scenario if it is running.
    ((SmallBankWorkload*)workload_.get())
        ->refreshSunflowerRegions(duration_.count(), elapsed_time().count() / 1000000LL);
  }
    ...
    if (api::Response res; RecvDeserializedProtoWithEmptyDelim(socket_, res)) {
      ...
      if (txn->status() == TransactionStatus::ABORTED) {
        if (txn->abort_code() == AbortCode::RESTARTED || txn->abort_code() == AbortCode::RATE_LIMITED) {
          ...
          IncAbortedTxns();
          // In the case of abort with no restart requested for dependent transactions,
          // the cause is a self-abort, so we don't want to stop the client.
          if (workload_->name() == "pps" &&
              info.profile.dependency_type == TransactionProfile::DependencyType::SECOND_PHASE) {
            if (!duration_reached) {
              SendNextTxn();
            }
          }
